...
export function useSessionController({ onOpenChat, onToast }: UseSessionControllerOptions) {
  ...
  function registerUserDisplayMessage(
    thread: ThreadRecord,
    ...
    const target =
      messages.find((message) => message.role === "user" && message.text === transportText) ??
  ...
  function registerSkillMessage(thread: ThreadRecord, transportText: string, displayText: string, skillName: string) {
    return registerUserDisplayMessage(thread, transportText, displayText, skillName);
  }
  ...
  function applyWorkspacePayload(payload: BootstrapPayload) {
    const fallbackActiveThread =
      payload.activeThreadId && localThreadOverridesRef.current[payload.activeThreadId]
        ? localThreadOverridesRef.current[payload.activeThreadId]
        : null;
    const currentThread = payload.currentThread
      ? preferLocalThreadState(payload.currentThread)
      : fallbackActiveThread;
  ...
  async function createThread() {
    ...
    try {
      ...
      if (payload.currentThread) {
        rememberLocalThreadOverride(payload.currentThread);
      }
    ...
    if (state.draft || !state.activeThreadId) {
      ...
      try {
        ...
        const nextThread = skillMeta
          ...
          : result.knowledge?.injected
            ? registerUserDisplayMessage(result.thread, result.knowledge.query, message)
            : decorateThread(result.thread);
        const localOverride = {
          ...optimisticThread,
          id: nextThread.id,
          title: nextThread.title || optimisticThread.title,
          archived: nextThread.archived,
          workspaceRoot: nextThread.workspaceRoot || draftWorkspaceRoot,
          updatedAt: Math.max(optimisticThread.updatedAt, nextThread.updatedAt),
        } satisfies ThreadRecord;

        dispatch({ type: "draft/set", payload: null });
        if (shouldKeepLocalThreadOverride(localOverride, nextThread)) {
          rememberLocalThreadOverride(localOverride);
        } else {
          clearLocalThreadOverride(nextThread.id);
