    ...
    <section className="report-shell">
      <aside className="report-sidebar">
        <div className="report-sidebar-head">
          ...
          <h2>项目环评分析</h2>
          <p>基于知识库、项目坐标和地图定位结果，自动生成可导出的环评报告章节。</p>
        </div>
        ...
        <div className="report-sidebar-card">
          <div className="report-inline-head">
            <strong>地图定位</strong>
            <span>{mapTools.length > 0 ? `检测到 ${mapTools.length} 个地图工具` : "暂未检测到地图工具"}</span>
          </div>

          {mapTools.length === 0 ? (
            <button
              type="button"
              className="secondary-button report-connect-button"
              onClick={() => void onAddMapTool()}
            >
              <MapPinned size={14} />
              接入高德地图 MCP
            </button>
        ...
        <div className="report-hero">
          <div className="report-hero-card">
            ...
            <div className="report-hero-copy">
              ...
              <span>报告生成会优先使用所选知识库中的法规、规划和项目材料。</span>
            </div>
          </div>
          <div className="report-hero-card">
            <div className="report-hero-icon">
            ...
            <div className="report-hero-copy">
              <strong>{form.longitude && form.latitude ? "坐标已就绪" : "可选坐标补充"}</strong>
              <span>有地图工具时会自动定位，没有时也会基于你输入的位置与坐标继续分析。</span>
            </div>
          </div>
          <div className="report-hero-card">
            <div className="report-hero-icon">
            ...
            <div className="report-hero-copy">
              ...
              <span>输出固定包含编制依据、评价等级、选址符合性分析和政策符合性分析。</span>
            </div>
          </div>
        </div>
        ...
        <div className="report-panel">
          {result ? (
            ...
            <div className="report-empty">
              <strong>先补全项目基础信息，再一键生成</strong>
              <span>
                这一栏会把知识库检索、地图定位和项目参数组织成一份更适合环评写作的报告草稿，并直接输出为
                Word 文档。
              </span>
            </div>
          )}
