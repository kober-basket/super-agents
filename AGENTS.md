...
npm run runtime:check:win
npm run package:runtime
npm run package:runtime:win
npm run preview
```
...
- Permission system：`PermissionManager` 根据 agent policy、工具风险、审批要求和 full filesystem access 做 allow/ask/deny。Runtime permission presets follow Codex-style boundaries: `default` keeps policy/user approval at the boundary, `smart-review` keeps the same boundary but routes eligible approval requests to an auto-reviewer agent, and `full-access` bypasses approval and workspace filesystem boundaries.
...
- 图片附件应按模型能力处理：当前会话模型声明支持视觉时直接随多模态请求发送；当前会话模型不支持视觉时，若设置 > 模型里的图片解析模型已启用，则先生成图片文字上下文，再把任务交回原会话模型继续；若未启用，则不做兜底并明确报错。若视觉模型实际拒绝图片输入，可以再走同一图片解析兜底；不能把图片解析模型当作任务模型静默切换。
