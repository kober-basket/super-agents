...
## Super Agents Runtime

不要在 skill 目录内创建 venv，也不要运行 `scripts/setup.sh` / `scripts/setup.bat`。Super Agents 内置 Python 和 uv，PDF 依赖按需安装到应用的 generated runtime 目录。

```bash
PYTHON="$(super-agents-document-runtime ensure pdfkit-core --print-python)"
"$PYTHON" scripts/pdfkit.py help
```

高级表格、压缩和 PDF 转 Word 能力需要时再安装：

```bash
super-agents-document-runtime ensure pdfkit-optional --print-python
```

公式识别 `pix2tex` 体积很大，只在用户明确需要公式识别时安装：

```bash
super-agents-document-runtime ensure pdfkit-formula --print-python
```
...
```bash
"$PYTHON" /path/to/pdfkit-py/scripts/pdfkit.py help
```

所有命令都用 `super-agents-document-runtime ensure ... --print-python` 返回的 Python 路径和 `scripts/pdfkit.py` 的绝对路径调用。
...
3. **如果依赖环境不存在**：先运行 `super-agents-document-runtime ensure pdfkit-core --print-python` 初始化环境。

```powershell
# ✅ 正确的 Windows PowerShell 调用方式
& $python "C:\path\to\pdfkit-py\scripts\pdfkit.py" split --input "D:\docs\file.pdf" --output_dir "D:\docs\output" --mode ranges --ranges "[[0,2]]"
